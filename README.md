...
- `shared_identity_expires_at` — the current shared cookie's absolute deadline (a `Time`), or `nil` when there's no valid shared cookie. It tracks the cookie, not `signed_in?`: an anonymous, preferences-only cookie (no `user_id`) still has a deadline and returns it here. Kept out of `current_shared_identity` so that Hash stays exactly your `allowed_claims` keys and can round-trip back into `write_shared_identity`.
